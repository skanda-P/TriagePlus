...
import { Send, Wifi, WifiOff, Loader2, Mic } from 'lucide-react';
...
function WsStatusPill({ status, isInitialConnect }: { status: WsStatus; isInitialConnect: boolean }) {
  ...
  if (isInitialConnect) return null;
...
export function ChatWindow({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    if (status === 'open') setIsInitialConnect(false);
  }, [status]);

  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages, isTyping]);

  const handleIntakeSubmit = (data: any) => {
    send({ type: 'intake_form', ...data });
    setShowIntake(false);
    setIsTyping(true);
  };
      ...
      <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-cards overflow-hidden border border-frost-gray dark:border-gray-800 transition-colors duration-300" style={{ boxShadow: 'var(--shadow-subtle)' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-frost-gray dark:border-gray-800 bg-lavender-mist dark:bg-slate-800 transition-colors duration-300">
          ...
          <div className="flex items-center gap-4">
            <StethoscopeToggle />
            <WsStatusPill status={status} isInitialConnect={true} />
          </div>
        </div>
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <Loader2 className="w-10 h-10 animate-spin mx-auto text-canopy-green dark:text-sky-signal" />
            <p className="mt-4 text-charcoal dark:text-white">Connecting to TriagePlus AI…</p>
            <p className="mt-2 text-sm text-slate-muted dark:text-ash">Establishing secure WebSocket connection</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-cards overflow-hidden border border-frost-gray dark:border-gray-800 transition-colors duration-300" style={{ boxShadow: 'var(--shadow-subtle)' }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-frost-gray dark:border-gray-800 bg-lavender-mist dark:bg-slate-800 transition-colors duration-300">
        ...
        <div className="flex items-center gap-4">
          <StethoscopeToggle />
          <WsStatusPill status={status} isInitialConnect={false} />
        </div>
      </div>
      ...
      <div id="chat-messages" className="flex-1 overflow-y-auto chat-scroll px-4 py-4 flex flex-col gap-3" role="log" aria-live="polite">
        {showIntake ? (
          <IntakeMiniForm onSubmit={handleIntakeSubmit} />
        ) : (
          <>

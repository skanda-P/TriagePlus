...
def load_rag_models():
    global _medquad_index, _conversations_index, _embeddings_model
    if _embeddings_model is None:
        try:
            ...
            if (faiss_dir / "medquad").exists():
                _medquad_index = FAISS.load_local(str(faiss_dir / "medquad"), _embeddings_model, allow_dangerous_deserialization=True)
            if (faiss_dir / "conversations").exists():
                _conversations_index = FAISS.load_local(str(faiss_dir / "conversations"), _embeddings_model, allow_dangerous_deserialization=True)
        except Exception as e:
            print("Failed to load RAG FAISS indices:", e)
...
def node_extract_symptoms(state: TriageState) -> Dict:
    ...
    if _ner_pipeline is not None:
        try:
            entities = _ner_pipeline(last_msg)
...
def node_next_question(state: TriageState) -> Dict:
    ...
    if _conversations_index is not None and top_qs:
        ...
        docs = _conversations_index.similarity_search(search_query, k=2)
        for doc in docs:
...
def node_explain(state: TriageState) -> Dict:
    ...
    if state.get("is_emergency"):
        ...
        if _medquad_index is not None and final_diagnosis:
            docs = _medquad_index.similarity_search(final_diagnosis, k=3)
...
def node_prompt_booking(state: TriageState) -> Dict:
...
def node_handle_booking(state: TriageState) -> Dict:
    last_msg = state["messages"][-1].content.lower()
    if "yes" in last_msg or "sure" in last_msg or "ok" in last_msg or "book" in last_msg or "y" in last_msg:
...
def node_fetch_slots(state: TriageState) -> Dict:
    supabase = get_supabase()
    slots_list = []
    msg = "I'm sorry, I couldn't find any available slots right now."
    if supabase:
        try:
            res = supabase.table("clinician_slot").select("id, start_time, doctor(name, specialty(name))").eq("status", "open").limit(3).execute()
            if res.data:
                slots_list = res.data
...
def node_confirm_slot(state: TriageState) -> Dict:
    last_msg = state["messages"][-1].content.lower()
    ...
    if supabase and state.get("patient_id"):
        try:
            supabase.table("clinician_slot").update({"status": "held"}).eq("id", slot_id).execute()
            res = supabase.table("appointment").insert({
                "patient_id": state["patient_id"],
                "slot_id": slot_id,
...
def node_process_payment(state: TriageState) -> Dict:
    last_msg = state["messages"][-1].content.lower()
    if "pay" in last_msg:

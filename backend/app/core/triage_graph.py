...
from ..db.supabase_client import get_supabase, get_anon_supabase
...
# Global KG name-to-id cache (populated lazily)
...
def evaluate_red_flags(symptoms: List[str], text: str) -> bool:
    ...
    if any(trigger in text_lower for trigger in STANDALONE_EMERGENCY):
        return True
    # Check combination triggers against raw text (not E_XX codes)
...
def node_emergency_check(state: TriageState) -> TriageState:
    ...
    # Reset intent so it's re-detected on follow-up turns
    state["intent"] = None
    return state
...
def fuzzy_match_doctor(text: str) -> Optional[dict]:
    """Fuzzy match a doctor name from the user's message."""
    try:
        from rapidfuzz import process, fuzz
        supabase = get_supabase()
        res = supabase.table("doctor").select("id, name, specialty_id, specialty!inner(name)").execute()
        doctors = {row["name"]: {"id": row["id"], "specialty_name": row["specialty"]["name"]} for row in res.data}
...
async def _load_xgboost_artifacts():
    """Load XGBoost model and preprocessing artifacts in a thread."""
    import os
    import pickle
...
async def _get_patient_demographics(patient_id: str) -> tuple:
    """Fetch patient age and gender in a thread."""
    supabase = get_supabase()
...
async def _get_kg_severity(pred_condition: str) -> int:
    """Get severity from KG in a thread."""
    kg = get_kg()
    severity = kg.get_condition_severity(pred_condition)
    return severity if severity is not None else 3


async def _get_kg_department(pred_condition: str) -> Optional[str]:
    """Get department from KG in a thread."""
    global _kg_condition_name_to_id
    kg = get_kg()
    if _kg_condition_name_to_id is None:
        _kg_condition_name_to_id = {
            cinfo.get("name", "").lower(): cid
            for cid, cinfo in kg.conditions.items()
        }
    condition_id = _kg_condition_name_to_id.get(pred_condition.lower())
    if condition_id:
        return kg.get_condition_specialty(condition_id)
    return None


async def node_classify(state: TriageState) -> TriageState:
    # Try to load XGBoost if it exists, otherwise use mock
    clf, mlb, le = await _load_xgboost_artifacts()
    ...
    if clf is not None and mlb is not None and le is not None:
        ...
        age, sex = await _get_patient_demographics(state["patient_id"])
        ...
        severity = await _get_kg_severity(pred_condition)
        ...
        department = await _get_kg_department(pred_condition)

        # Fallback: Use symptom-to-department mapping
        if not department:
    ...
    if state["confidence"] < confidence_floor:
        state["triage_level"] = min(state["triage_level"] or 3, 3)
...
async def node_explain(state: TriageState) -> TriageState:
    import time
    t0 = time.time()
    retriever = get_unified_retriever()
...
# The compiled graph with checkpointer will be created in main.py lifespan
